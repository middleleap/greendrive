      ...
      {CONFIGURATOR_STEPS.map((step, i) => {
        ...
        return (
          <div key={step.id} className="flex items-center">
            ...
            <span
              className={`hidden sm:inline text-[10px] font-medium ml-1 ${isActive ? 'text-green-deep' : isCompleted ? 'text-bank-gray-mid' : 'text-bank-gray'}`}
            >
              {step.shortLabel}
            </span>
            {i < CONFIGURATOR_STEPS.length - 1 && (
              <div
                className={`cfg-step-connector ${i < currentStep ? 'cfg-step-connector-done' : ''}`}
              />
            )}
          </div>
        );
